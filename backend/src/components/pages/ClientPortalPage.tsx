...
export const ClientPortalPage: React.FC = () => {
  ...
  const clientCompany = {
    name: firstClient?.companyName || user?.department || 'Client Organization',
    slaLevel: 'STANDARD ADVISORY',
    leadPartner: firstClient?.assignedPartner || 'Senior Managing Partner',
    contactPerson: firstClient?.contactName || user?.name || 'Authorized Representative',
    email: firstClient?.contactEmail || user?.email || 'contact@client.com',
    phone: firstClient?.contactPhone || 'N/A',
    assetsManaged: firstClient?.retainerValue ? `${(firstClient.retainerValue * 12).toLocaleString()}` : '$0',
    contractRenewal: 'Active',
  };
      ...
      {selectedTab === 'meetings' && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="bg-[#0f172a] border border-slate-800 rounded-3xl p-6 shadow-xl space-y-4">
            ...
            <p className="text-xs text-slate-400">Direct escalation contacts assigned to your account.</p>
            ...
            <div className="space-y-3 pt-1">
              <div className="p-3.5 bg-slate-950/80 border border-slate-800 rounded-2xl flex items-center gap-3">
                ...
                />
                <div>
                  <p className="text-xs font-extrabold text-white">Managing Partner</p>
                  <p className="text-[10px] font-bold text-amber-400">Senior Practice Lead</p>
                  <p className="text-[10px] text-slate-400">admin@gamcs.com</p>
                </div>
              </div>
            </div>
          </div>
