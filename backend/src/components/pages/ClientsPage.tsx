...
export const ClientsPage: React.FC = () => {
  ...
  const [clientDetailTab, setClientDetailTab] = useState<'overview' | 'services' | 'emails'>('overview');
          ...
          <div className="w-full max-w-3xl bg-slate-900 border border-slate-800 rounded-3xl p-6 sm:p-8 shadow-2xl relative max-h-[90vh] overflow-y-auto custom-scrollbar select-none">
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 pb-6 border-b border-slate-800">
              ...
              <div className="flex items-center gap-2">
                <button
                  onClick={() => {
                    setEmailComposerClient(detailClient);
                    setIsEmailComposerOpen(true);
                  }}
                  className="px-3 py-1.5 bg-blue-600 hover:bg-blue-500 text-white text-xs font-bold rounded-xl shadow flex items-center gap-1.5 transition-all cursor-pointer"
                >
                  <Mail className="w-3.5 h-3.5" /> Send Email
                </button>
                <span className="px-3 py-1.5 text-xs font-black uppercase rounded-xl border bg-emerald-950 text-emerald-300 border-emerald-800">
            ...
            {/* Navigation Tabs */}
            <div className="flex items-center gap-2 border-b border-slate-800 my-4 pb-2">
              <button
                onClick={() => setClientDetailTab('overview')}
                className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all cursor-pointer ${
                  clientDetailTab === 'overview' ? 'bg-emerald-600 text-white' : 'text-slate-400 hover:text-white'
                }`}
              >
                Overview & Metrics
              </button>
              <button
                ...
                className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all cursor-pointer flex items-center gap-1.5 ${
                  clientDetailTab === 'services' ? 'bg-emerald-600 text-white' : 'text-slate-400 hover:text-white'
                }`}
              >
                <Briefcase className="w-3.5 h-3.5" /> Active Services ({(detailClient.services || []).length})
              </button>
              <button
                onClick={() => setClientDetailTab('emails')}
                className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all cursor-pointer flex items-center gap-1.5 ${
                  clientDetailTab === 'emails' ? 'bg-emerald-600 text-white' : 'text-slate-400 hover:text-white'
                }`}
              >
            ...
            {/* TAB CONTENT: EMAILS */}
            {clientDetailTab === 'emails' && (
              <div className="py-4">
                <EmailHistorySection
                  clientId={detailClient.id || detailClient.clientId}
                  recipientEmail={detailClient.email}
                  recipientName={detailClient.contactPerson || detailClient.name}
      ...
      {emailComposerClient && (
        <EmailComposerModal
          ...
          onClose={() => {
            ...
            setEmailComposerClient(null);
          }}
          initialRecipient={emailComposerClient.email || ''}
          recipientName={emailComposerClient.contactPerson || emailComposerClient.name}
          clientId={emailComposerClient.id || emailComposerClient.clientId}
          clientName={emailComposerClient.name}
          onEmailSent={(sentEmail) => {
            addToast(
              sentEmail.status === 'SENT' ? 'success' : 'info',
              sentEmail.status === 'SENT' ? 'Email Dispatched' : 'Email Logged',
              `Email "${sentEmail.subject}" logged in Client communications history.`
            );
          }}
        />
      )}
