...
import {
  User, AuthResponse, Lead, Client, ClientService, Contact, Meeting, Project, Task, DocumentItem, Proposal, Invoice,
  DashboardStats, AIInsight, ApiConfig, ActivityItem, NotificationItem, RolePermission, RoleItem, FirmSettings, ReportData,
  Remark, EmailRecord, SendEmailPayload, SmtpStatusResponse, OutlookAccountStatus, OutlookConnectResponse
} from '../types';
...
export const apiService = {
  ...
  // --- EMAIL MANAGEMENT REST ENDPOINTS ---
  sendEmail: async (payload: SendEmailPayload): Promise<EmailRecord> => {
    const client = createApiClient();
    const res = await client.post('/emails/send', payload);
    return res.data?.data || res.data;
  },
  ...
  getLeadEmails: async (leadId: string): Promise<EmailRecord[]> => {
    const client = createApiClient();
    const res = await client.get(`/leads/${leadId}/emails`);
    return res.data?.data || res.data;
  },

  getClientEmails: async (clientId: string): Promise<EmailRecord[]> => {
    const client = createApiClient();
    const res = await client.get(`/clients/${clientId}/emails`);
    return res.data?.data || res.data;
  },

  getContactEmails: async (contactId: string): Promise<EmailRecord[]> => {
    const client = createApiClient();
    const res = await client.get(`/contacts/${contactId}/emails`);
    return res.data?.data || res.data;
  },

  resendEmail: async (id: string): Promise<EmailRecord> => {
    const client = createApiClient();
    const res = await client.post(`/emails/${id}/resend`, {});
    return res.data?.data || res.data;
  },

  getSmtpStatus: async (): Promise<SmtpStatusResponse> => {
    const client = createApiClient();
    const res = await client.get('/emails/smtp-status');
    return res.data?.data || res.data;
  },

  getOutlookStatus: async (): Promise<OutlookAccountStatus> => {
    const client = createApiClient();
    const res = await client.get('/outlook/status');
    return res.data?.data || res.data;
  },

  getOutlookConnectUrl: async (): Promise<OutlookConnectResponse> => {
    const client = createApiClient();
    const res = await client.get('/outlook/connect');
    return res.data?.data || res.data;
  },

  sendOutlookEmail: async (payload: SendEmailPayload): Promise<EmailRecord> => {
    const client = createApiClient();
    const res = await client.post('/emails/send', { ...payload, provider: 'OUTLOOK' });
    return res.data?.data || res.data;
  },

  disconnectOutlook: async (): Promise<any> => {
    const client = createApiClient();
    const res = await client.post('/outlook/disconnect', {});
    return res.data?.data || res.data;
  }
};
