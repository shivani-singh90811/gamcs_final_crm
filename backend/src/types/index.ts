...
export type TaskStatus = 'PENDING' | 'IN_PROGRESS' | 'UNDER_REVIEW' | 'COMPLETED' | 'ON_HOLD' | 'CANCELLED' | 'BACKLOG' | 'IN_REVIEW';
...
export type ContractFormatOption =
  | 'STANDARD_PROPOSAL'
  | 'RETAINER_CONTRACT'
  | 'MSA_AGREEMENT'
  | 'SOW_DELIVERABLE'
...
export interface ContractFormatDetail {
  value: string;
  label: string;
  badge: string;
  color: string;
...
export interface OutlookAccountStatus {
  isConnected: boolean;
  configured: boolean;
