...
export const LoginPage: React.FC = () => {
  ...
  // Sign in state
  const [email, setEmail] = useState('s.jenkins@archicorp-cfo.com');
  const [password, setPassword] = useState('cfo_partner_2026');
  ...
  const handleDemoLogin = async (roleKey: 'super_admin' | 'admin' | 'employee' | 'client') => {
    setError(null);
    let demoEmail = '';
    let demoPass = 'cfo_partner_2026';
    
    if (roleKey === 'super_admin') {
      demoEmail = 's.jenkins@archicorp-cfo.com';
    } else if (roleKey === 'admin') {
      demoEmail = 'm.chen@archicorp-cfo.com';
    } else if (roleKey === 'employee') {
      demoEmail = 'r.black@archicorp-cfo.com';
    } else if (roleKey === 'client') {
      demoEmail = 'm.vance@starlightbio.io';
      ...
      <div className="max-w-md w-full relative z-10 my-8">
        ...
        <div className="bg-[#0f172a] border border-slate-800 rounded-3xl shadow-2xl overflow-hidden">
          ...
          <div className="p-6">
            ...
            {/* Quick Demo Logins Bar (4 Enterprise Roles) */}
            <div className="mt-6 pt-5 border-t border-slate-800">
              <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-2.5 text-center flex items-center justify-center gap-1">
                <Sparkles className="w-3.5 h-3.5 text-amber-400" /> Test 4 Enterprise Roles Instantly
              </p>
              <div className="grid grid-cols-2 gap-2">
                <button
                  type="button"
                  onClick={() => handleDemoLogin('super_admin')}
                  className="p-2.5 bg-indigo-950/80 hover:bg-indigo-900 border border-indigo-800/80 rounded-xl text-left transition-all cursor-pointer group"
                >
                  <div className="text-[11px] font-black text-indigo-300 flex items-center gap-1">
                    <span>👑 Super Admin</span>
                  </div>
                  <div className="text-[9px] text-indigo-400/90 font-medium truncate mt-0.5">Company Owner</div>
                </button>

                <button
                  type="button"
                  onClick={() => handleDemoLogin('admin')}
                  className="p-2.5 bg-purple-950/80 hover:bg-purple-900 border border-purple-800/80 rounded-xl text-left transition-all cursor-pointer group"
                >
                  <div className="text-[11px] font-black text-purple-300 flex items-center gap-1">
                    <span>🛡️ Admin / Manager</span>
                  </div>
                  <div className="text-[9px] text-purple-400/90 font-medium truncate mt-0.5">Senior Partner</div>
                </button>

                <button
                  type="button"
                  onClick={() => handleDemoLogin('employee')}
                  className="p-2.5 bg-emerald-950/80 hover:bg-emerald-900 border border-emerald-800/80 rounded-xl text-left transition-all cursor-pointer group"
                >
                  <div className="text-[11px] font-black text-emerald-300 flex items-center gap-1">
                    <span>💼 Employee</span>
                  </div>
                  <div className="text-[9px] text-emerald-400/90 font-medium truncate mt-0.5">Financial Analyst</div>
                </button>

                <button
                  type="button"
                  onClick={() => handleDemoLogin('client')}
                  className="p-2.5 bg-amber-950/80 hover:bg-amber-900 border border-amber-800/80 rounded-xl text-left transition-all cursor-pointer group"
                >
                  <div className="text-[11px] font-black text-amber-300 flex items-center gap-1">
                    <span>🏢 Client</span>
                  </div>
                  <div className="text-[9px] text-amber-400/90 font-medium truncate mt-0.5">CFO Client Portal</div>
                </button>
              </div>
            </div>
          </div>
