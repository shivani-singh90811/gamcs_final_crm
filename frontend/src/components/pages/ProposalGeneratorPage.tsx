...
export const ProposalGeneratorPage: React.FC = () => {
  ...
  const [clientName, setClientName] = useState('Meridian Real Estate Holdings');
  const [projectTitle, setProjectTitle] = useState('Q3 Portfolio DCF Valuation & Lease Audit');
  const [fee, setFee] = useState(280000);
  const [engagementType, setEngagementType] = useState('Valuation & Advisory');
  const [leadPartner, setLeadPartner] = useState('Sarah Jenkins');
  const [scopeDetails, setScopeDetails] = useState(
    '1. DCF Valuation model across 14 commercial assets.\n2. ASC 842 audit readiness analysis.\n3. Executive board presentation deck.'
  );
